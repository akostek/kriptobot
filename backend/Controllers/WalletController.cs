...
namespace KriptoBot.API.Controllers
{
    ...
    public class WalletController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetWallet()
        {
            try
            {
                ...
                var enrichedBalances = balances.Select(b =>
                {
                    ...
                    if (openTrades.Any())
                    {
                        ...
                        var firstTrade = openTrades.OrderBy(t => t.CreatedAt).First();
                        if (firstTrade.Side == "BUY")
                        {
                            pnlUsdt = ((double)currentPrice - avgBuyPrice) * totalAmount;
                            pnlPercent = (pnlUsdt / totalCost) * 100;
                        }
                    }
            ...
            {
                if (ex.Message.Contains("credentials not found"))
                {
        ...
        {
            return Ok(new {
                demo = Binance.Net.BinanceEnvironment.Demo.SpotRestAddress,
                testnet = Binance.Net.BinanceEnvironment.Testnet.SpotRestAddress,
                live = Binance.Net.BinanceEnvironment.Live.SpotRestAddress
            });
        }
